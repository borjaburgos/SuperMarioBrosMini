	...
	switch(current_behavior){
		...
		switch(actor_states[i]){
			case 0:
				...
				CLR_FLAG(right_bowser_hand->flags, ACTOR_FLAG_COLLISION);
				CLR_FLAG(left_bowser_hand->flags, ACTOR_FLAG_COLLISION);
				break;
			case 1: //Intro
				if (!(game_time & 1)){
					if (actor->pos.y > 1408){
						...
						SET_FLAG(right_bowser_hand->flags, ACTOR_FLAG_COLLISION);
						SET_FLAG(left_bowser_hand->flags, ACTOR_FLAG_COLLISION);
			...
			case 8: //Debris fall
				...
				if (debris_idx != 0 && (actor_states[debris_idx] == 0 || actor_states[debris_idx] == 255)){
					...
					if (!CHK_FLAG(debris_actor->flags, ACTOR_FLAG_ACTIVE)){
						CLR_FLAG(debris_actor->flags, ACTOR_FLAG_DISABLED);
